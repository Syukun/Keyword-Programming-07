...
public class FunctionTree implements Comparable<FunctionTree>{
	 ...
	 */
	public FunctionTree(Function f, List<String> input_words, HashMap<String, Integer> numOfSameWordsIn){
		...
		e_vec = ExplanationVector.calcExplanationVector(input_words, numOfSameWordsIn, this);
	}
	...
	public String toCompleteMethodString(){
		...
		if(isStatic == true && funcType.equals("method")){
			...
			//クラス名.メソッド名となる。
			//rootの名前
			if(!root.getFunction().getParentClass().equals("this"))
			...
			//rootの前に書く
			if(!root.getFunction().getParentClass().equals("this"))//(親がthis除く。
